...
//==== Task States ============================================================
#define TASK_RUNNING \
  0x00 ///< The process is either: 1) running on CPU or 2) waiting in a run queue.
#define TASK_INTERRUPTIBLE \
  (1 << 0) ///< The process is sleeping, waiting for some event to occur.
#define TASK_UNINTERRUPTIBLE \
  (1 << 1) ///< Similar to TASK_INTERRUPTIBLE, but it doesn't process signals.
#define TASK_STOPPED \
  (1 << 2) ///< Stopped, it's not running, and not able to run.
#define TASK_TRACED \
  (1 << 3) ///< Is being monitored by other processes such as debuggers.

...
void pci_scan_func(pci_scan_func_t f, int type, int bus, int slot, int func,
                   void *extra) {
...
void pci_scan_slot(pci_scan_func_t f, int type, int bus, int slot,
                   void *extra) {
...
void pci_remap() {
  ...
  if (pci_isa) {
    dprintf("PCI-to-ISA interrupt mappings by line:\n");

    for (int i = 0; i < 4; ++i) {
      pci_remaps[i] = pci_read_field(pci_isa, 0x60 + i, 1);
      dprintf("\tLine %d: 0x%2x\n", i + 1, pci_remaps[i]);
    }
...
int pci_get_interrupt(uint32_t device) {
  ...
  if (irq_pin == 0) {
    dprintf("PCI device does not specific interrupt line\n");
    return pci_read_field(device, PCI_INTERRUPT_LINE, 1);
  }
  ...
  dprintf("Slot is %d, irq pin is %d, so pirq is %d and that maps to %d?"
          "int_line=%d\n",
          pci_extract_slot(device), irq_pin, pirq, pci_remaps[pirq], int_line);

  if (pci_remaps[pirq] == 0x80) {
    dprintf("Not mapped, remapping?\n");
...
static void __scan_count(uint32_t device, uint16_t vendorid, uint16_t deviceid,
                         void *extra) {
  (void)device;
  (void)vendorid;
  (void)deviceid;

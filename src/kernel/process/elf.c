...
static inline elf_section_header_t *
elf_get_section_header_table(elf_header_t *header) {
...
static inline elf_program_header_t *
elf_get_program_header_table(elf_header_t *header) {
...
static inline elf_section_header_t *elf_get_section_header(elf_header_t *header,
                                                           unsigned idx) {
...
/// @return a pointer to the desired section header.
static inline elf_program_header_t *elf_get_program_header(elf_header_t *header,
                                                           unsigned idx) {
...
elf_get_section_header_string_table(elf_header_t *header) {
  if (header->shstrndx == SHT_NULL)
    return NULL;
  return (
    const char *)((uintptr_t)header +
                  elf_get_section_header(header, header->shstrndx)->offset);
}
...
/// @return a pointer to the section header string table, or NULL on failure.
static inline const char *
elf_get_symbol_string_table(elf_header_t *header,
                            elf_section_header_t *section_header) {
  if (section_header->link == SHT_NULL)
    return NULL;
  return (
    const char *)((uintptr_t)header +
                  elf_get_section_header(header, section_header->link)->offset);
}
...
static inline const char *
elf_get_section_header_name(elf_header_t *header,
                            elf_section_header_t *section_header) {
...
/// @return a pointer to the section header string table, or NULL on failure.
static inline const char *
elf_get_symbol_name(elf_header_t *header, elf_section_header_t *section_header,
                    elf_symbol_t *symbol) {
...
static inline elf_section_header_t *
elf_find_section_header(elf_header_t *header, const char *name) {
  for (unsigned i = 0; i < header->shnum; ++i) {
    ...
    const char *section_header_name =
      elf_get_section_header_name(header, section_header);
...
static inline elf_symbol_t *elf_find_symbol(elf_header_t *header,
                                            const char *name) {
  for (unsigned i = 0; i < header->shnum; ++i) {
    // Get the section header.
    elf_section_header_t *section_header = elf_get_section_header(header, i);
    ...
    if (section_header && (section_header->type == SHT_SYMTAB)) {
      ...
      elf_symbol_t *symtab =
        (elf_symbol_t *)((uintptr_t)header + section_header->offset);
      // Iterate the entries.
      for (unsigned j = 0; j < symtab_entries; ++j) {
        ...
        const char *symbol_name =
          elf_get_symbol_name(header, section_header, symbol);
        if (symbol_name) {
...
static inline void elf_dump_section_headers(elf_header_t *header) {
  ...
  for (unsigned idx = 0; idx < header->shnum; ++idx) {
    ...
    dprintf("[%2d] %-20s %-15s %08x %06x %06x %2u %3u %2u %3u %2u\n", idx,
            section_header_name,
            elf_section_header_type_to_string(section_header->type),
            section_header->addr, section_header->offset, section_header->size,
            section_header->entsize, section_header->flags,
  ...
  for (unsigned i = 0; i < header->shnum; ++i) {
    ...
    for (int j = 0; j < symtab_entries; ++j) {
      ...
      const char *symbol_name =
        elf_get_symbol_name(header, section_header, symbol);
      if (symbol_name == NULL) {
        dprintf("Null symbol name.\n");
        return;
      }
      // Dump the symbol.
      dprintf("[%4d] %08x %5d %-7s %-6s %-8s %3d %s\n", j, symbol->value,
              symbol->size,
...
static inline int elf_load_exec(elf_header_t *header, task_struct *task) {
  dprintf(" Type      | Mem. Size | File Size | VADDR\n");
...
int elf_load_file(task_struct *task, vfs_file_t *file, uint32_t *entry) {
  ...
  if (vfs_fstat(file, &stat_buf) < 0) {
    dprintf("Failed to stat the file `%s`.\n", file->name);
    return false;
  }
  ...
  if (buffer == NULL) {
    dprintf(
      "Failed to allocate %d bytes of memory for reading the file `%s`.\n",
  ...
  if (vfs_read(file, buffer, 0, stat_buf.st_size) != stat_buf.st_size) {
    dprintf("Failed to read %d bytes from the file `%s`.\n", stat_buf.st_size,
            file->name);
  ...
  dprintf("Entry          : 0x%x\n", header->entry);
  dprintf("Headers offset : 0x%x\n", header->phoff);
  dprintf("Headers count  : %d\n", header->phnum);
  // Check the elf header.
  if (!elf_check_file_header(header)) {
    dprintf("File %s is not a valid ELF file.\n", stat_buf.st_size, file->name);
    goto return_error_free_buffer;
  }
  // Check if the elf file is an executable.
  if (header->type != ET_EXEC) {
    dprintf("Elf file is not an executable.\n");
    goto return_error_free_buffer;
  }
  if (!elf_load_exec(header, task)) {
    dprintf("Failed to load the executable.\n");
    goto return_error_free_buffer;
  }
...
int elf_check_file_header(elf_header_t *header) {
  ...
  if (header->ident[EI_CLASS] != ELFCLASS32) {
    dprintf("Unsupported ELF File Class.\n");
    return false;
  }
  if (header->ident[EI_DATA] != ELFDATA2LSB) {
    dprintf("Unsupported ELF File byte order.\n");
    return false;
  }
  if (header->machine != EM_386) {
    dprintf("Unsupported ELF File target.\n");
    return false;
  }
  if (header->ident[EI_VERSION] != EV_CURRENT) {
    dprintf("Unsupported ELF File version.\n");
    return false;
  }
  if (header->type != ET_EXEC) {
    dprintf("Unsupported ELF File type.\n");
    return false;
  }
  return true;
}
...
int elf_check_magic_number(elf_header_t *header) {
  ...
  if (header->ident[EI_MAG0] != ELFMAG0) {
    dprintf("ELF Header EI_MAG0 incorrect.\n");
    return false;
  }
  if (header->ident[EI_MAG1] != ELFMAG1) {
    dprintf("ELF Header EI_MAG1 incorrect.\n");
    return false;
  }
  if (header->ident[EI_MAG2] != ELFMAG2) {
    dprintf("ELF Header EI_MAG2 incorrect.\n");
    return false;
  }
  if (header->ident[EI_MAG3] != ELFMAG3) {
    dprintf("ELF Header EI_MAG3 incorrect.\n");
    return false;
  }
  return true;
}

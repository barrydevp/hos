...
static const char *str_weekdays[] = { "Sunday",    "Monday",   "Tuesday",
                                      "Wednesday", "Thursday", "Friday",
                                      "Saturday" };

static const char *str_months[] = { "January", "February", "March",
                                    "April",   "May",      "June",
...
time_t sys_time(time_t *time) {
  ...
  t = (365 * curr_time.tm_year) + (curr_time.tm_year / 4) -
      (curr_time.tm_year / 100) + (curr_time.tm_year / 400);
  // Convert months to days
  t += (30 * curr_time.tm_mon) + (3 * (curr_time.tm_mon + 1) / 5) +
       curr_time.tm_mday;
